...
6. How to Configure the Juypter Notebook
...
The infrastructure we are setting up will allow your local computer to communicate with the AWS servers remotely.  In order to do this, you will need to create a secure key pair that will make it possible for the AWS servers to confirm your identity when you open a remote session.  

The key consists of two parts -- a public key and a private key.  Your computer needs to know both.  AWS needs to know the public key.  The local private key will interface with the AWS public key to confirm your identity to the system.  This pair is known as *ssh* for "secure shell."
...
Once the AWS account is set up and available, sign in to the console and go to the *AWS Services* section.  There, you will find under *All Services* a link for *EC2*, which stands for Elastic Compute Cloud.  Click on the *EC2* link and you will be directed to the *EC2* control panel.

**PRO-TIP**  *Before starting the set up, look at the bar at the top of your browser.  On the right will be your name and a georgraphic area.  You are setting up your system in a particular geographic server area, so make sure you pick one and stick with it.  You cannot access your system if the setting is pointing towards a different area.*
...
Copy your public key from the text editor and paste it into the *Public key contents* box.  Make sure you capture the entire key.  If you are satisfied you copied it correctly, click *Import* and you are done.  Don't worry if there is an error.  You can always delete this key and generate a new key pair following the method described above.

## Step 4:  Create a New Security Group

Go back to the *EC2* Dashboard and click on *Security Groups*.  You are going to configure this next.

Click *Create Security Group*.  Give your group a good name and a useful description but leave the VPC (Virtual Private Cloud) at its default setting.  Next, choose the Inbound tab where you will set five Rules for your configuration.  These Rules will apply to *SSH*, *HTTP*, *Jupyter*, *Docker Hub*, and *Mongo*.  The only changes you make will be to the columns *Type*, *Port Range*, *Source*, and *Description*.
...
AWS offers up to 30 GiB of EBS General Purpose (SSD) storage free of charge each month.  In keeping with our free tier eligiblity, change the "Size (GiB)" selection to 30.  Leave all other choices as default.
...
Go back to the shell on the local computer.  At the command prompt, type the following and paste the IP address you copied from AWS after the @:
...
As mentioned earlier, if the AWS server is stopped the existing IP address will not be available when it is started up again.  A new IP address will be issued. This means you have to restart the Jupyter Notebook.  Unfortunately, this is not as simple as pasting the new IP address into the browser bar.  You must repeat a series of commands.  Please follow instructions above for these commands and remember to use the new IP address from AWS, the new container ID, and the new passcode string generated in the shell:
...
Then copy and paste the url into your web browser. Be sure to update *localhost* with the new IP address.
